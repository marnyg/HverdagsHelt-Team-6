...
/**
 * Form component for region modification.
 */
class EditRegionForm extends Component {
  ...
  /**
   * Updates components properties' region object.
   * @param newProps Component properties to be set
   */
  ...
  /**
   * When component mounts: Fetch counties from server.
   */
  mounted() {
  ...
  /**
   * When submitting form: check basic HTML5 form validity and send region object to server.
   * @param event
   */
  submit(event) {
    ...
    if (form.checkValidity()) {
      ...
      regionService
        .updateRegion(newRegion, newRegion.region_id)
        .then(res => {
          console.log(res);
          $('#spinner').hide();
