...
/**
 * Form component for editing an employee user.
 */
class EditUserForm extends Component {
  ...
  render() {
    return (
      <div key={this.props.user.user_id}>
        ...
        <button onClick={event => this.submit(event)} type="button" className="btn btn-primary">
          Oppdater
        </button>
      </div>
    );
  ...
  /**
   * Updates components properties if user differs between states.
   * @param newProps Properties to be set.
   */
  ...
  /**
   * Component unmounts: Logging function.
   */
  ...
  /**
   * Component mounts: Logging function.
   */
  mounted() {
  ...
  /**
   * Validates form using basic HTML5 validation and sends user object to database if the form is valid.
   * @param event
   */
  submit(event) {
