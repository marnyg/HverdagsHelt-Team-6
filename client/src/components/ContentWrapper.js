...
class ContentWrapper extends Component {
  ...
  render() {
    if (this.location) {
      if (this.cases !== null && this.cases.length > 0) {
        // No response from the locationservice has been given, wait before showing
        $('#loader').hide();
        return (
          <div>
            <Content
  ...
  fetch_cases(location: Location, resolve_cb, reject_cb) {
    ...
    caseService
      .getCasesByLoc(location.city, location.region)
      .then((cases: Case[]) => {
        resolve_cb(cases);
      })
      .catch((error: Error) => {
        ...
        .then((location: Location) => {
          ...
          caseService
            ...
            .catch((error: Error) => {
              console.error(error);
              $('#spinner').hide();
              this.cant_find_loaction = <NoLocationPage onSubmit={region_id => this.onRegionSelected(region_id)} />;
            });
        })
        .catch(error => {
          console.error(error);
  ...
  loadResults() {
    ...
    caseService
      ...
      .catch((error: Error) => {
        console.error(error);
        Notify.warning(
          'Det oppstod en feil ved lasting av saker. Vennligst prøv igjen. Hvis problemet vedvarer kontakt oss. \n\nFeilmelding: ' +
            error.message
        );
        this.nextPageButton.disabled = false;
      });
    ...
    return (
      <div className={'container mt-5'}>
        <div className={'row'}>
          ...
          <div className={'col w-100 text-center'}>
            <h3 className={'w-100'}>Side: {this.page}</h3>
          </div>
          <div className={'col w-100 text-center'}>
            <button
              ref={e => {
                this.nextPageButton = e;
              }}
              className={'btn btn-primary w-100'}
              onClick={() => {
                this.nextPageButton.disabled = true;
